...
> **종합**: near-OOD는 현 프로젝트 **최대 난제로 확정**(far와 큰 격차). CREE variance가 **유일 작동 레버(0.782)**지만 *moderate* — forensic-confident 거부(0.95+) 미달이므로 *완화 가능 한계*로 정직하게 frame. set-size·거리는 약함(negative/weak).
