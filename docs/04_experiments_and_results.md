...
법과학 ancestry의 표준 도구 **Snipper** (Phillips et al. 2007, *FSI:Genetics* 1:273 [verify page])는 AIM-SNP 빈도 위 **naive Bayes 분류기**다. naive Bayes = 좌위 독립 가정의 **log-linear allele-frequency 분류기** — 우리 **MultinomialNB**와 같은 클래스.
→ **우리 증거**: MultinomialNB가 **82.2%로 최고 정확도**(LogReg 79.6·LinearSVC 79.8보다 위, §24.4) — *수십 년 forensic 관행의 빈도-기반 naive-Bayes 모델이 이 과제에서 near-optimal*임을 정량 재확인. (Snipper vs 현대 ML 비교: *Advancing biogeographical ancestry predictions through machine learning*, FSI:G 2025 [verify] — TabPFN 등과 대조.)
...
**참고문헌(검증됨, [verify]=서지 세부 재확인)**: Pritchard et al. 2000 *Genetics* 155:945 · Patterson, Price & Reich 2006 *PLoS Genet* 2:e190 · Price et al. 2006 *Nat Genet* 38:904 · Novembre & Stephens 2008 *Nat Genet* 40:646 (doi:10.1038/ng.139) · Alexander, Novembre & Lange 2009 *Genome Res* 19:1655 · Phillips et al. 2007 (Snipper, *FSI:G* 1:273) [verify] · Romero et al. 2017 (Diet Networks, *ICLR*) · Gorishniy et al. 2021 (*NeurIPS*) · Shwartz-Ziv & Armon 2022 *Inf. Fusion* 81:84–90 (arXiv:2106.03253) · Grinsztajn, Oyallon & Varoquaux 2022 (*NeurIPS* D&B, arXiv:2207.08815) · *Advancing biogeographical ancestry predictions through ML* FSI:G 2025 [verify].
...
- **(b) embedding + conformal** (`scripts/trust/47`): RandEff를 Mondrian conformal로 wrapping → **coverage 0.920 유지**(trust layer는 embedding-DL에도 model-agnostic), set 1.91; 단 far-OOD AUROC 0.649(LogReg 0.840보다 약) → embedding의 MSP open-set이 약함을 확인 → (d)가 이를 *내재 분산*으로 개선 시도.
- **(c) SSL + random-effects encoder** (`scripts/ssl/48`, gnomAD): *[진행]* — 정규화가 대규모 SSL을 살리나(vs naive SSL 54%).
- **(d) variance-as-nonconformity** (`scripts/trust/49`): *[진행]* — 변분 random-effects 분산 점수 open-set AUROC vs MSP.
- **(e) 극소표본** (`scripts/models/50`): *[진행]* — n=50–full에서 RandEff vs LogReg 저하 곡선.
